...
```{r}
file_remove <- function(file_path) {
  if (!file.exists(file_path)) {
    rlang::abort("specified file path does not exist!")
  }
  invisible(file.remove(file_path))
}

invisible(file.create("test.txt"))

# File exists
file_remove(file_path = "test.txt")

# File does not exist
file_remove(file_path = "test2.txt")
```

2. The `appendLF` argument of `message()` indicates whether you would like a new line to be created after the end of your call to `message()` (i.e. it is equivalent to the idea in most programming languages of `print(\n)`)
...
```{r}
abort_pkg_missing <- function(pkg) {
  msg <- glue::glue("Cannot load `{pkg}`! Install package first with `install.packages()`")
  rlang::abort("err_pkg_missing",
               message = msg,
               pkg = pkg)
}
...
load_pkg <- function(pkg) {
  if (!pkg %in% installed.packages()) {
    abort_pkg_missing(pkg = pkg)
  }
  base::requireNamespace(pkg, quietly = F)
}

pkg_cnd <- load_pkg(pkg = "abc") %>% rlang::catch_cnd()

str(pkg_cnd)
```

### Part 10: Function factories ###

## Section 10.2 ##

3. 

Creation of a factory to pick element `n` (specified by the user) of a given data structure
...
Computes the `i`th central moment of an (empirical) numeric vector:
